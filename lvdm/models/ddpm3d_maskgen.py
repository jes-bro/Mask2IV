...
from utils.utils import instantiate_from_config, matches_temporal
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.first_stage_extra_key = first_stage_extra_key
    ...
    def get_loss(self, pred, target, mean=True, mask=None, fg_weight=5):
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        self.val_kwargs = val_kwargs if val_kwargs else {}
    ...
    def apply_model(self, x_noisy, t, cond, return_all=False, **kwargs):
    ...
    def p_losses(self, x_start, cond, t, noise=None, **kwargs):
        ...
        # breakpoint()
        mask = (kwargs['target_mask'] + 1) / 2.0
        mask_sum = mask.sum(1)
        mask_fg = F.interpolate(mask_sum, scale_factor=1/8, mode='nearest').unsqueeze(1)
        ...
        loss_simple = self.get_loss(model_output, target, mean=False, mask=mask_fg).mean([1, 2, 3, 4])
        ...
        if self.original_elbo_weight != 0.0:
            loss_vlb = self.get_loss(model_output, target, mean=False, mask=mask_fg).mean(dim=(1, 2, 3, 4))
...
class LatentVisualDiffusion(LatentDiffusion):
    ...
    def shared_step(self, batch, random_uncond, **kwargs):
        ...
        x, c, fs, target_mask, _ = self.get_batch_input(batch, random_uncond=random_uncond, return_fs=True, return_original_input=True)
        kwargs.update({"fs": fs.long()})
        kwargs.update({"target_mask": target_mask})
        loss, loss_dict = self(x, c, **kwargs)
        return loss, loss_dict
    
    def get_batch_input(self, batch, random_uncond, return_first_stage_outputs=False, return_original_cond=False, return_fs=False, return_cond_frame=False, return_original_input=False, return_obj_info=False, **kwargs):
        ## x: b c t h w
        x = super().get_input(batch, self.first_stage_key)
        
        if self.first_stage_extra_key is not None:
            x_extra = super().get_input(batch, self.first_stage_extra_key)
            z_extra = self.encode_first_stage(x_extra[:, :, 0]) # first image latent
            # mask first, img second
        ...
        if self.model.conditioning_key == 'hybrid':
            if self.interp_mode:
                ...
                # img_cat_cond = torch.zeros_like(z_cat)
                # img_cat_cond[:,:,0,:,:] = z_cat[:,:,0,:,:]
                # img_cat_cond[:,:,-1,:,:] = z_cat[:,:,-1,:,:]

                # for bd v2
                img_cat_cond = torch.zeros_like(z)
                img_cat_cond[:,:,0,:,:] = z[:,:,0,:,:]
                img_cat_cond[:,:,-1,:,:] = last_mask_z
                img_latent = z_extra.unsqueeze(2)
                img_latent = repeat(img_latent, 'b c t h w -> b c (repeat t) h w', repeat=z.shape[2])
                img_cat_cond = torch.cat([img_cat_cond, img_latent], dim=1)
    ...
    def val_preprocess(self, x):
        # normalize to [0, 1]
        x = x.clamp(-1, 1).float().permute(0, 2, 1, 3, 4)
        return (x + 1) / 2    
    
    @torch.no_grad()
    def validation_step(self, batch, batch_idx):
        _, loss_dict_no_ema = self.shared_step(batch, random_uncond=self.classifier_free_guidance)
        self.log_dict(loss_dict_no_ema, prog_bar=True, logger=True, on_step=True, on_epoch=True, sync_dist=True)
        batch_logs = self.log_images(batch, **self.val_kwargs)
        return batch_logs
    
    def validation_epoch_end(self, outputs):
        x_samples = torch.cat([x['samples'] for x in outputs])  # N x 3 x 16 x h x w
        x_recs = torch.cat([x['reconst'] for x in outputs])
        
        # compute metrics
        x_samples, x_recs = self.val_preprocess(x_samples), self.val_preprocess(x_recs)
        mse = (x_samples - x_recs).pow(2).mean()
        miou = self.compute_miou(x_samples, x_recs)
        self.log('mse', mse, prog_bar=True, logger=True, sync_dist=True)
        self.log('miou', miou, prog_bar=True, logger=True, sync_dist=True)
        mainlogger.info(f"Validation result [mse: {mse: .4f}, miou: {miou: .4f}].")
    
    def compute_miou(self, pred, target, threshold=0.5, eps=1e-6):
        """
        Compute mean IoU between prediction and target tensors.
        
        Args:
            pred (torch.Tensor): shape (bs, 3, 16, 320, 512), float, values in [0, 1]
            target (torch.Tensor): same shape as pred
            threshold (float): threshold to binarize predictions
            eps (float): small constant to avoid division by zero

        Returns:
            float: mean IoU over all elements and channels
        """
        ...
        intersection = (pred_bin * target_bin).sum(dim=(2, 3, 4))  # shape: (bs, 3)
        union = ((pred_bin + target_bin) > 0).float().sum(dim=(2, 3, 4))  # shape: (bs, 3)

        iou = (intersection + eps) / (union + eps)  # shape: (bs, 3)
    ...
    def log_images(self, batch, sample=True, ddim_steps=50, ddim_eta=1., plot_denoise_rows=False, \
        ...
        sampled_img_num = kwargs['sampled_img_num']
        if sampled_img_num is not None:
            for key in batch.keys():
                batch[key] = batch[key][:sampled_img_num]
        ...
        z, c, xc, fs, cond_x, x_ori, videos, objs = self.get_batch_input(batch, random_uncond=False,
                                                      return_original_cond=True,
                                                      return_fs=True,
                                                      return_cond_frame=True,
                                                      return_original_input=True,
                                                      return_obj_info=True)
        ...
        log["caption"] = xc
        xc_with_fs = []
        for idx, (content, obj) in enumerate(zip(xc, objs)):
        ...
        if sample:
            ...
            if unconditional_guidance_scale != 1.0:
                ...
                img = torch.zeros_like(x_ori[:,:,0]) ## b c h w
                ## img: b c h w
                img_emb = self.embedder(img) ## b l c
    ...
    def configure_optimizers(self):
        ...
        # freeze the temporal weights if temporal_freeze is enabled
        freeze_num = 0
        for name, param in self.model.named_parameters():
            if self.freeze_temporal and matches_temporal(name):
                param.requires_grad = False            
                freeze_num += 1
        
        params = [p for p in self.model.parameters() if p.requires_grad == True]
        mainlogger.info(f"@Training [{len(params)}/{len(params) + freeze_num}] of Full Paramters.")
