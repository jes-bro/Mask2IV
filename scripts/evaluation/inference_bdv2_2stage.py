...
from lvdm.data.bdv2 import format_mask
...
def load_data_prompts(data_dir, save_dir, video_size=(256,256), video_frames=16, interp=False, second_stage=False):
    transform = transforms.Compose([
        transforms.Resize(min(video_size), antialias=True),
    ...
    for idx in range(n_samples):
        if interp:
            ...
            if not second_stage:
                ...
                mask1 = cv2.cvtColor(cv2.imread(file_list[idx].replace('.jpg', '_start.png')), cv2.COLOR_BGR2RGB)
                mask2 = cv2.cvtColor(cv2.imread(file_list[idx].replace('.jpg', '_end.png')), cv2.COLOR_BGR2RGB)
                mask_tensor1 = torch.tensor(mask1).permute(2, 0, 1).unsqueeze(1).float()
                mask_tensor2 = torch.tensor(mask2).permute(2, 0, 1).unsqueeze(1).float()
                # mask_tensor1 = repeat(mask_tensor1, 'c t h w -> c (repeat t) h w', repeat=video_frames)
                # mask_tensor2 = repeat(mask_tensor2, 'c t h w -> c (repeat t) h w', repeat=video_frames)
                mask_tensor1 = transform(format_mask(mask_tensor1))
                mask_tensor2 = transform(format_mask(mask_tensor2))
                mask_tensor = torch.cat([mask_tensor1, mask_tensor2], dim=1)    # c x 2 x h x w
            else:
                ...
                # TODO: to check, transform may not need in this case
    ...
    return filename_list, data_list, mask_list, fps_list, fs_list
...
def save_results_seperate(samples, filename, fakedir, fps=10, loop=False, mask=False):
...
def image_guided_synthesis(model, videos, masks, noise_shape, n_samples=1, ddim_steps=50, ddim_eta=1., \
    ...
    if first_stage:
        # set the actor mask of the end mask to be background (-1)
        masks[:, 1, -1] = -1
        z_mask = get_latent_z(model, masks)

        img_cat_cond = torch.zeros_like(z)
        img_cat_cond[:,:,0,:,:] = z_mask[:,:,0,:,:]
        img_cat_cond[:,:,-1,:,:] = z_mask[:,:,-1,:,:]
        img_latent = z[:,:,0].unsqueeze(2)
...
def run_inference(args, gpu_num, gpu_no):
    ...
    filename_list, data_list, mask_list, fps_list, fs_list = load_data_prompts(args.prompt_dir, fakedir_separate, video_size=(args.height, args.width), video_frames=n_frames, interp=args.interp, second_stage=args.second_stage_only)
    ...
    if not args.second_stage_only:
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            for idx, indice in tqdm(enumerate(range(0, len(filename_list_rank), args.bs)), desc='Sample Batch'):
                ...
                batch_samples = image_guided_synthesis(model1, videos, masks, noise_shape, args.n_samples, args.ddim_steps, args.ddim_eta,
                ...
                for nn, samples in enumerate(batch_samples):
                    ...
                    save_results_seperate(samples, filename, fakedir, fps=8, loop=args.loop, mask=True)
    ...
    with torch.no_grad(), torch.cuda.amp.autocast():
        for idx, indice in tqdm(enumerate(range(0, len(filename_list_rank), args.bs)), desc='Sample Batch'):
            ...
            batch_samples = image_guided_synthesis(model2, videos, masks, noise_shape, args.n_samples, args.ddim_steps, args.ddim_eta, 
            ...
            for nn, samples in enumerate(batch_samples):
                ...
                filename = filenames[nn]
                # save_results(prompt, samples, filename, fakedir, fps=8, loop=args.loop)
                save_results_seperate(samples, filename, fakedir, fps=8, loop=args.loop)
