...
class ImageLogger(Callback):
    def __init__(self, batch_frequency, val_batch_frequency=1, max_images=8, clamp=True, rescale=True, save_dir=None, \
                to_local=False, to_img=False, log_images_kwargs=None):
        super().__init__()
        ...
        if self.to_local:
            ## default save dir
            self.save_dir = os.path.join(save_dir, "images" + "_cfg" + str(self.log_images_kwargs.get("unconditional_guidance_scale", "")))
    ...
    def log_to_tensorboard(self, pl_module, batch_logs, filename, split, save_fps=8):
        ...
        action = "|".join(batch_logs["condition"])
        for key in batch_logs:
            value = batch_logs[key]
            tag = "gs%d-%s/%s-%s-%s"%(global_step, split, filename, action, key)
            ...
            if isinstance(value, torch.Tensor) and value.dim() == 5:
                ...
                img = value
                grid = torchvision.utils.make_grid(img, nrow=int(img.shape[0]), padding=0)
    ...
    @rank_zero_only
    def log_batch_imgs(self, pl_module, batch, batch_idx, split="train", batch_log_data=None):
        ...
        if batch_log_data is None:
            with torch.no_grad():
                log_func = pl_module.log_images
                batch_logs = log_func(batch, split=split, **self.log_images_kwargs)
        else:
            sampled_img_num = self.log_images_kwargs.get("sampled_img_num", 1)
            for key in batch_log_data.keys():
                batch_log_data[key] = batch_log_data[key][:sampled_img_num]
            batch_logs = batch_log_data
        ...
        if self.to_local:
            ...
            filename = f"{batch_idx}"
            log_local(batch_logs, os.path.join(self.save_dir, split), filename, save_fps=8, to_img=self.to_img)
        else:
            mainlogger.info("Log [%s] batch <%s> to tensorboard ..."%(split, filename))
            self.log_to_tensorboard(pl_module, batch_logs, filename, split, save_fps=8)
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=None):
        ...
        batch_freq = self.batch_freq // trainer.num_devices
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx=None):
        ...
        if batch_idx == 0 and trainer.is_global_zero:
            print("VAL outputs keys:", outputs.keys() if isinstance(outputs, dict) else type(outputs), flush=True)
        ...
        if self.val_batch_freq != -1:
            ...
            if (batch_idx+1) % batch_freq == 0 and pl_module.logdir:
                self.log_batch_imgs(pl_module, batch, batch_idx, split="val", batch_log_data=outputs)
